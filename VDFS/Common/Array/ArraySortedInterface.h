...
namespace Common {
  template <class T, int( __cdecl* FUNC )(const T&, const T&)>
  ...
  template <class T, int( __cdecl* FUNC )(const T&, const T&)>
  uint ArraySorted<T, FUNC>::Remove( const T& obj ) {
    return Remove<FUNC>( obj );
  }



  template <class T, int( __cdecl* FUNC )(const T&, const T&)> template<void* LAMBDA, class O>
  uint ArraySorted<T, FUNC>::Remove( const O& obj ) {
    uint Index = FindIndexForObject<FUNC>( obj );
  ...
  template <class T, int( __cdecl* FUNC )(const T&, const T&)> template<void* LAMBDA, class O>
  uint ArraySorted<T, FUNC>::SearchEqual( const O& obj ) const {
    if( GetNum() == 0 )
      return Invalid;
  ...
  uint ArraySorted<T, FUNC>::FindIndexForObject( const O& object ) const {
    ...
    while( true ) {
      int pivot = (l + r) >> 1;
      int cmp = FUNC( object, allocator[pivot] );
      if( cmp == 0 )
        return pivot;
...
#pragma warning(disable:4244)
  ...
  void ArraySorted<T, FUNC>::QuickSort( uint low, uint hight ) {
    ...
    T pivot = allocator[(i + j) >> 1];
    byte temp[sizeT];

    while( i <= j ) {
      while( FUNC( pivot, allocator[i] ) > 0 )
        i++;

      while( FUNC( pivot, allocator[j] ) < 0 )
        j--;
