...
namespace Common {
  ...
  template <class T>
  uint Array<T>::Remove( const T& obj ) {
  ...
  bool Array<T>::CompareInsorted( const Array& other ) const {
    ...
    for( uint i = 0; i < size1; i++ ) {
      const T& Object  = GetAllocator()[i];
      uint Equals1 = CountOfEqual( Object );
      uint Equals2 = other.CountOfEqual( Object );
      if( Equals1 != Equals2 )
        return false;
    }
  ...
  uint Array<T>::SearchEqualSorted( const O& obj ) const {
    ...
    auto foundObj = GetAllocator()[Index];
    if( Compare( foundObj, obj ) == 0 )
      return Index;
  ...
  uint Array<T>::FindIndexForObject( const O& object ) const {
    ...
    while( true ) {
      int pivot = (l + r) >> 1;
      int cmp = Compare( object, allocator[pivot] );
      if( cmp == 0 )
        return pivot;
...
#pragma warning(disable:4244)
  ...
  void Array<T>::QuickSort( uint low, uint hight ) {
    ...
    while( i <= j ) {
      while( Compare( pivot, allocator[i] ) > 0 /*allocator[i] < pivot*/ )
        i++;

      while( Compare( pivot, allocator[j] ) < 0 /*allocator[j] > pivot*/ )
        j--;
